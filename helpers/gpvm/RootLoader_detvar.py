...
def load_this_sample(dir_path, folder, root_files):
    start = time.time()
    ...
    for fn in root_files:
        ...
        sample_type = max([e for e in enum_keys if e in sample_name], key=len)
        ...
        print("\t", fn, sample_name, sample_enum, period)
        ...
        if folder not in [
            ...
            sample_info["pot"][(sample_enum, period)] = this_pot
            # Create the truth array
            truth = load_truth_event(uproot_file[main_tree], period, sample_enum)
        ...
        sample_info["daughters"] = this_daughters
        sample_info["numentries"] = numentries
        sample_info["fields"] = this_fields
        if truth:
            sample_info["mc"] = truth
        pickle.dump(
            sample_info, open(dir_path +folder+ "/slimmed/{}_slimmed.pckl".format(sample_name), "wb")
        )
    end = time.time()
...
for folder in sub_folders:
    ...
    load_this_sample(dir_path, folder, root_files)
print("Done!")
