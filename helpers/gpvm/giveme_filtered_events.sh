...
runsubrunlist=/uboone/app/users/wvdp/EventDisplay/nueccinc_sel_18May20_16ect.txt

# The name of the sam def which you made the run subrun event list from
originalsamdef_r1=data_bnb_mcc9.1_v08_00_00_25_reco2_C1_beam_good_reco2_5e19
originalsamdef_r3=data_bnb_mcc9.1_v08_00_00_25_reco2_G1_beam_good_reco2_1e19

# choose a name for your sam def, any name as long as it doesnt already exist
newsamdef=wouter_nueccinc_sel_19May20
...
for f in ${runsubrunlist_arr[@]}; do
    ...
    for line in $(cat $f); do
        ...
        if [ $counter -eq 0 ]; then
                ...
                samweb list-files "(defname:$originalsamdef_r1 or defname:$originalsamdef_r3) and run_number $runsubrun ${query_array[${type}]}" | tee -a files_$(basename $f)
        fi
